...
5. **Log scrubbing** — log messages with substrings matching `api_key=`, `token=`, `password=`, `bearer` followed by a space, or PEM headers must be scrubbed by the log writer.
