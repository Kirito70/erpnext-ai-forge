...
- **POS pricing rules are profile-derived (NPOS-F2).** `ignore_pricing_rule` and `apply_discount_on` are derived server-side from the linked POS Profile (`_set_pos_profile_pricing_fields`) and stripped from every client payload (`_strip_client_system_fields`) — a client can neither force nor suppress rule application; the POS Profile `ignore_pricing_rule` Check is the per-profile opt-out (AC3). `quote_pos_pricing` (`novizna_pos.novizna_pos.invoice.quote_pos_pricing`, whitelisted, `@rate_limit` 20/min per caller, read-only via a rolled-back savepoint) returns the authoritative rule-applied totals the server WILL book; the cart displays those and captures `grand_total` so an offline capture does not false-conflict at sync. The fail-closed `_assert_pricing_rules_did_not_change_totals` (tolerance 0.02) runs on save/sync/force-sync after the save computes rule-applied totals, and only when a rule actually applied (item `pricing_rules` JSON, transaction discount evidence, or a matching active Transaction rule — the fallback mirrors ERPNext's own lookup, `apply_discount_on` is NOT a filter and empty-company rules DO re-arm it). Divergence (incl. a missing client total) raises `pricing rule changed the invoice total` → `_classify_sync_error` maps it to the dedicated **`pricing_changed`** conflict category (non-retryable, manager review; `pricing_tax` remains the generic pricing-error category). Manual sale discounts fall back to client computation; manual LINE discounts on rule-affected items always route to `pricing_changed` review (server rule overrides the line discount — fail-closed, intentional).
- **Pricing Rule editor parity (NPOS-D14).** The console edits the full promotion surface: `item_groups`/`brands` targeting + `customer`/`territory`/`campaign`/`customer_group` audience (driven by `applicable_for`, server nulls the others), the Product buy-X-get-Y scheme, and conditions/margin/priority. Server-side, `masters/pricing_rule_policy.py` is the single guard module (wired pre-save on create+update) — it is the ONLY place pricing-rule coherence is enforced and it is stricter than upstream ON PURPOSE: `priority` is a validated Select 1..20 (crafted values refused), `free_qty` is capped at `FREE_ITEM_MAX_QTY = 9999` (mirrors F2's `QUOTE_MAX_ITEM_QTY`; upstream applies `free_qty` uncapped and divides by `recurse_for` — both refused before persistence, incl. legacy stored divisors via the `is_recursive` re-arm leg), negative `discount_percentage`/`discount_amount`/`min_qty`/`max_qty`/`min_amt`/`max_amt` are refused (upstream refuses only `Rate`), margin requires a type, and a Product rule must resolve a free item. `apply_on`/`price_or_product_discount`/`selling`/`currency`/`company` are create-only. Client-side, `MasterEditorPanel` takes an **opt-in `visibilityConfig` prop** (a resolver `(form) => { sections, fields }`) — the ONLY consumer is `PosManagePricingRuleRecordPage` via `src/config/pricingRuleVisibility.ts`; with no prop the panel renders byte-identical flat stacks for every other consumer (NPOS-X1 gate). D14b's checkout proof (free-item lines land in the invoice) is gated on gap ticket **NPOS-E24** (F2's integration tests cover Price rules only) — do not claim it closed until E24 lands.
...
8. **Tabbed mode (NPOS-X1)**: `MasterEditorPanel` accepts `use-tabs` and `show-record-tabs` (both default `false`). The **default flat stack must stay visually identical** for existing consumers — never add unconditional rules to the shared `.item-editor__head` (scope under its `--tabs` modifier). Tabbed mode renders only the active layout tab's sections via the shared `RecordTabStrip`, replicates `RecordDetail`'s vanished-tab repair watch, auto-activates the tab holding the first errored field on validation failure, and — with `show-record-tabs` on an existing record — appends Connections/Activity tabs reusing `RecordConnections`/`RecordActivity`. Known EPIC-X caveat: a doctype with exactly one layout tab + record tabs currently has no pill back to the sections once Connections is active.
9. **Do NOT migrate**: `ItemEditorPanel.vue` (1698 ln — has stock/valuation/UOM compositing logic), `CompanyEditorPanel.vue` (433 ln — has perpetual-inventory guard), `TaxCategoryEditorPanel.vue`, `ModeOfPaymentEditorPanel.vue` (out of scope for U5).
...
5. **Test layout** — non-DocType tests live under `novizna_pos/novizna_pos/tests/`, grouped
   ...
   - `tests/fixtures/` — shared builders; test-only, guarded against non-test runs. **Fixture profile prefix contract (NPOS-D31):** test-created POS Profiles MUST be named with a `NPOS-` or `POS-SEED-` prefix. The repair-scenario cascade (`repair_scenarios._close_test_owned_sessions`, called before every fresh `ensure_opening_entry` open) closes any Open POS Opening Entry the user holds on a `NPOS-%`/`POS-SEED-%` profile — including sessions leaked by earlier suites — so a profile outside these prefixes can trip ERPNext's per-user cashier guard ("Cashier is currently assigned to another POS") and is NOT auto-cleaned. Never open till sessions on a non-prefixed profile in tests.
...
- **`RecordReportLinks.vue` is the one component that renders record → report deep
  links** (NPOS-E20). Link definitions live in the backend catalog (`ReportEntry.record_links`,
  per-record `RecordLink` with scoping filter keys + a date policy — `window` from/to or
  `point_in_time` as-of key). The component renders ONLY links declared for the record's
  doctype in the fetched (capability-filtered) catalog — never hardcoded filter strings, never
  a disabled placeholder for an omitted/excluded link. A report whose filters change is caught
  by the AC#9 walk (`test_every_record_link_scoping_filter_is_declared_in_filter_schema`), not
  by hunting the UI.
